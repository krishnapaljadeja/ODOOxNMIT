...
export function ProductFilters({
  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div
        initial={false}
        animate={{ height: isFiltersOpen || window.innerWidth >= 768 ? "auto" : 0 }}
      ...
      >
        <div className="flex flex-col md:flex-row gap-4 p-4 md:p-0 bg-card md:bg-transparent rounded-lg md:rounded-none border md:border-none">
          ...
          <div className="space-y-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full md:w-auto justify-between bg-transparent">
                  {selectedCategory}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full md:w-auto justify-between bg-transparent">
                  ...
                  {groupByOptions.find((opt) => opt.value === groupBy)?.label || "No Grouping"}
                  <ChevronDown className="h-4 w-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent className="w-56">
          ...
          {/* Active Filters */}
          {(selectedCategory !== "All" || sortBy !== "newest" || groupBy !== "none") && (
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {sortBy !== "newest" && (
                <Badge variant="secondary" className="cursor-pointer" onClick={() => onSortChange("newest")}>
                  {sortOptions.find((opt) => opt.value === sortBy)?.label} ×
                </Badge>
              )}
              {groupBy !== "none" && (
                <Badge variant="secondary" className="cursor-pointer" onClick={() => onGroupByChange("none")}>
